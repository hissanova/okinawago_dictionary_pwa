...
import { IonAccordion, IonAccordionGroup, AccordionGroupChangeEventDetail, AccordionGroupCustomEvent } from '@ionic/react';
...
const SearchResults = ({ results, searchTerm }: any) => {
  const printAccordionState = (ev: Event): void => {
    const event = ev as AccordionGroupCustomEvent;
    console.log(event);
    console.log(event.detail)
    if (event.detail.value) console.log(results[event.detail.value]);
  };
  return (
    <>
      {
        results.length > 0 ?
          <IonAccordionGroup onIonChange={printAccordionState}>
